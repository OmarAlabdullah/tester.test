...
{
	$('#results_table').empty();
	$('#results_table').append('<tr><th>Postcode</th><th>Huisnummer</th><th>Toevoeging</th><th>Type</th><th>Bestand</th><th>Match</th></tr>');
	var matchted = 0;
	var unmatchted = 0;
	for(i in arr)
	{
		...
		if(!arr[i]['matched'] || !arr[i]['type_matched'])
		{
			...
			matchted++;
		$('#results_table').append('<tr ' + (this_matched ? 'class="matched"' : 'class="unmatched"') + '><td>' + arr[i]['zipcode'] + '</td><td>' + arr[i]['homenumber'] + '</td><td>' + arr[i]['addition'].toUpperCase() + '</td><td>' + arr[i]['type'] + ' ' + (arr[i]['new'] ? 'nieuwe leiding' : '') + '</td><td><a href="/tests/read_file_from_zip?zip_filename=' + filename + '&filename=' + arr[i]['filename'] + '" target="_blank">' + arr[i]['filename'] + '</td><td>' + (arr[i]['matched'] ? 'ja' : 'nee') + '</td></tr>');
	}
	if(unmatchted == 0)
	{
		$('#results_table').hide();
