	...
	if(!is_null($controller['layout']) && !empty($controller['layout']))
	{
		foreach($controller['css'] as $css)
			if(is_file(CSS_ROOT . $css))
				$css_for_layout .= '<link href="' . WEBROOT . CSS_ROOT . $css . ($controller['developer'] == 2 ? '?' . time() : ($controller['developer'] == 1 ? '?' . $controller['revision'] : '')) . '" rel="stylesheet" type="text/css">';
		foreach($controller['js'] as $js)
			if(is_file(JS_ROOT . $js))
				$js_for_layout .= '<script src="' . WEBROOT . JS_ROOT . $js . ($controller['developer'] == 2 ? '?' . time() : ($controller['developer'] == 1 ? '?' . $controller['revision'] : '')) . '" type="text/javascript"></script>';
